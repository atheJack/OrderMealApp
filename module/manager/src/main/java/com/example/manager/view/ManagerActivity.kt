...
class ManagerActivity: BaseActivity<ManagerViewModel>() {
    ...
    private fun initObserver() {
        viewModel.foodListData.observe(this, {
            if(it.code == 200) {
                if (adapter == null) {
                    adapter = FoodListAdapter(this, it.data as ArrayList<Food>, object : CommonItemClickListener<Food>{
                        override fun onClick(itemView: View, data: List<Food>, position: Int) {
                            ...
                            editFoodBt.setOnClickListener {
                                Navigation.jumpWithBundleForResult(this@ManagerActivity, Router.FOOD_EDIT, foodEditCode, Bundle().apply {
